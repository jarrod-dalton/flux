...
#> List of 4
#>  $ variables      :List of 4
#>   ..$ route_zone   :List of 7
#>   ..$ battery_pct  :List of 7
#>   ..$ payload_kg   :List of 7
#>   ..$ dispatch_mode:List of 7
#>  $ time_spec      :List of 8
#>   ..$ unit            : chr "hours"
#>   ..$ zone            : chr "UTC"
#>   ..$ origin          : POSIXct[1:1], format: "1970-01-01"
#>   ..$ origin_class    : chr "POSIXct"
#>   ..$ origin_posix    : POSIXct[1:1], format: "1970-01-01"
#>   ..$ origin_date     : Date[1:1], format: "1970-01-01"
#>   ..$ days_per_unit   : num 0.0417
#>   ..$ seconds_per_unit: num 3600
#>   ..- attr(*, "class")= chr "time_spec"
#>  $ decision_points:List of 1
#>   ..$ :List of 9
#>   .. ..- attr(*, "class")= chr "DecisionPoint"
#>  $ decision_groups: NULL
dispatch_schema$decision_points[[1]]
#> <DecisionPoint: dispatch_response >
#>   trigger        : dispatch_check
#>   condition      : (none)
#>   audit          : FALSE
#>   on_pending_act : warn
#>   allowed_actions: confirm_dispatch, decline_dispatch
#>   label          : Respond to an open delivery offer
```
...
|:-------------------------------|:----------------|
|offer_time                      |1                |
|state_after_offer               |assigned         |
|selected_action                 |decline_dispatch |
|selected_for_time               |1.5              |
|matching_action_type_in_history |FALSE            |
...
|              |Offer event    |Action event     |
|:-------------|:--------------|:----------------|
|time          |1.0            |1.5              |
|event_type    |dispatch_check |decline_dispatch |
|dispatch_mode |assigned       |idle             |
|payload_kg    |4              |0                |
...
|:--------------------|:-----------------|
|t                    |1                 |
|decision_point_id    |dispatch_response |
|trigger_event        |dispatch_check    |
|selected_action      |decline_dispatch  |
|dispatch_mode_before |idle              |
|dispatch_mode_after  |assigned          |
|payload_kg_before    |0                 |
|payload_kg_after     |4                 |
...
|                 |Healthy battery |Low battery      |
|:----------------|:---------------|:----------------|
|starting_battery |80              |15               |
|condition_met    |FALSE           |TRUE             |
|selected_action  |NA              |decline_dispatch |
...
|                |Dispatch response |Battery safety |
|:---------------|:-----------------|:--------------|
|t               |1                 |1              |
|condition_met   |TRUE              |TRUE           |
|selected_action |confirm_dispatch  |shed_payload   |
...
|              |After shed payload |After confirm dispatch |
|:-------------|:------------------|:----------------------|
|time          |1.25               |1.50                   |
|event_type    |shed_payload       |confirm_dispatch       |
|dispatch_mode |assigned           |in_transit             |
|payload_kg    |2                  |2                      |
|battery_pct   |15                 |15                     |
...
|scenario        |  t|decision_point_id |condition_met |selected_action  |
|:---------------|--:|:-----------------|:-------------|:----------------|
|healthy battery |  1|dispatch_response |TRUE          |confirm_dispatch |
|healthy battery |  1|battery_safety    |FALSE         |NA               |
|low battery     |  1|dispatch_response |TRUE          |confirm_dispatch |
|low battery     |  1|battery_safety    |TRUE          |shed_payload     |
...
|                          |Dispatch response    |Battery safety       |
|:-------------------------|:--------------------|:--------------------|
|decision_point_id         |dispatch_response    |battery_safety       |
|grouped_decision_point_id |post_dispatch_review |post_dispatch_review |
|group_activation_id       |group_activation_1   |group_activation_1   |
|condition_met             |TRUE                 |TRUE                 |
|selected_action           |confirm_dispatch     |shed_payload         |
...
#> $strategy
#> [1] "confirm with reduced payload"
```
