...
delivery_transition_with_policy <- function(entity, event, param_ctx = NULL) {
  ...
  if (identical(event$event_type, "dispatch_check")) {
    ...
    return(list(
      ...
      payload_kg    = rlnorm(1, meanlog = log(3.0), sdlog = 0.35)
    ))
  }
  ...
  if (identical(event$event_type, "accept")) {
    ...
    battery_drop <- rexp(1, rate = 1 / 2.5)
  ...
  # All other events (delivery_completed, end_shift) use the base transition.
...
#> Condition not met (battery >= 25, skipped):   4
```
