...
> **Note:** `set_schema()` currently manages state variables only and does not
> accept a `decision_points` argument. The list-construction form above is the
> intended pattern until `set_schema()` is extended to support decision points.
...
It receives the decision point object and the current courier state (already
updated by the triggering event's transition) and returns an `ActionEvent`, or
`NULL` for "no intervention."
