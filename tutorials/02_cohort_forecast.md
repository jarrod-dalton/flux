...
- `run_id` is the batch-local join key for each realized run. The same value
  names the run and appears in its callback context and trajectory records, so
  outputs from this batch can be joined without relying on list position.
