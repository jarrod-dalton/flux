...
```{r}
# Extend the schema with a priority_mode variable
schema_dp <- set_schema(
  vars = list(
    route_zone           = list(type = "categorical",
                                levels = c("urban", "suburban", "rural")),
    battery_pct          = "percent",
...
dp_bundle <- list(
  ...
  transition = function(entity, event, ctx = NULL) {
    ...
    if (identical(event$event_type, "surge")) {
      return(list(priority_mode = "surge"))
    }
    if (identical(event$event_type, "stand_down")) {
      return(list(priority_mode = "normal"))
    }
    list()
  },
...
# Attach the decision point to the schema
schema_dp$decision_points <- list(dp_post_dispatch)
